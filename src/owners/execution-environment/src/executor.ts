...
export class BoundedExecutorService {
  private readonly handlers = new Map<string, CustomActionHandler>();
  constructor(private readonly storage: StorageSubstrateService) {}

  registerHandler(actionId: string, handler: CustomActionHandler): void {
    if (typeof actionId !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(actionId) || typeof handler !== "function") {
      throw new Error("A custom handler requires a valid action ID and callable handler.");
    }
    if (this.handlers.has(actionId)) {
      throw new Error(`Handler ${actionId} is already registered.`);
    }
  ...
  private async runBoundedAction(input: BoundedExecutorSubmission): Promise<BoundedExecutorOutputRecord[]> {
    if (input.actionKind === "custom") {
      const handler = this.handlers.get(input.actionId);
      if (!handler) throw new Error(`No handler is registered for ${input.actionId}.`);
      // The handler receives a copy so mutation cannot change the saved envelope.
      const outputs = await handler(structuredClone(input));
      if (!Array.isArray(outputs) || outputs.length > 100 || outputs.some((output) =>
        !output || typeof output.label !== "string" || typeof output.mediaType !== "string" || typeof output.body !== "string"
      )) throw new Error("Custom handlers must return at most 100 label/mediaType/body string outputs.");
      if (Buffer.byteLength(JSON.stringify(outputs), "utf8") > 1_000_000) {
        throw new Error("Custom handler output exceeds one megabyte.");
      }
      return outputs.map((output) => ({
        outputId: randomUUID(), label: output.label, mediaType: output.mediaType,
        body: output.body, createdAt: new Date().toISOString()
      }));
    }
